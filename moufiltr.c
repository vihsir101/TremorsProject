...
VOID FIR_Init(FIRFilter* ffilter)
{
    ffilter->index = 0;
    int baseWeight = 1024 / numTaps
    ;
    ...
    ffilter->coefficients[numTaps / 2] += 1024 % numTaps;

}

long FIR_Push(FIRFilter* ffilter, long change)
{
    ffilter->buffer[ffilter->index] = change; 
    ffilter->index = (ffilter->index + 1) % numTaps;

    ULONG sum = 0;
    ULONG bufIndex = ffilter->index;
    for (int i = 0; i < numTaps; i++) {
        bufIndex = (bufIndex - 1 + numTaps) % numTaps;
        sum += ffilter->coefficients[i] * ffilter->buffer[bufIndex];
    }
